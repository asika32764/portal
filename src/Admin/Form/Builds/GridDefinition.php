	...
	protected function doDefine(Form $form)
	{
		...
		$this->group('search', function (Form $form)
		{
			...
			$this->list('field')
				...
				->option(Translator::translate('admin.build.field.title'), 'build.title')
				->option(Translator::translate('admin.build.field.alias'), 'build.alias');
		 ...
		 *
		 * You can override filter actions in BuildsModel::configureFilters()
		 */
		$this->group('filter', function(Form $form)
		{
			// State
			$this->list('build.state')
				->label('State')
				// Add empty option to support single deselect button
				->option('', '')
				->option(Translator::translate('admin.build.filter.state.select'), '')
